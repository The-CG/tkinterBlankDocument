...
class starting_Or_Menu_Page:      # Grid Mode
    def __init__(self, master):
        ...
        self.value2 = tk.DoubleVar()    # Float
        #self.value3 = tk.StringVar()   # String
        #self.value4 = tk.IntVar()      # Integer
        
        # Creates entry feild
        self.firstValueEntry = tk.Entry(self.frame, textvariable = self.value2).grid(row = 2, column = 0)
    ...
    def sqrt2FacCommand(self):
        ...
        value1 = float(self.firstValueEntry.get())
        
        valueForThisCommand = value1 ^ (4*3*2*1)
        
        value2 = math.sqrt(value1)

        self.resultLabel = tk.Label(self, text = str(value1))
